...
// if any1 needs a simple way to calculate damage: atk*4+def+legionpower*5
	...
	window.SRDotDX = {
		...
		config: (function(){
			...
			tmp.showStatusOverlay = (typeof tmp.showStatusOverlay == 'boolean'? tmp.showStatusOverlay:true);
			...
			if(el){
				...
				if(SRDotDX.config.useMaxRaidCount && diff > 0){
					if(!SRDotDX.gui.Importing){
						...
						var i=0, total=0;
						var draids = SRDotDX.gui.GetNukedRaids();
		...
		gui: {
			...
			GetNukedRaids: function () {
				var r = [];
				var rds = this.GetAllRaids();
				for(i=0; i<rds.length; i++){
					var rd = rds[i];
					if(rd.nuked) r.push(rd);
				}
				return r;
			},
